  ...
  describe('visit()', () => {
    ...
    describe('options', () => {
      ...
      describe('baseUrl option', () => {
        it('should be stored in context', async () => {
          const baseUrl = 'https://www.example.com'
          mstr = ministr({ baseUrl }, context)
          expect(context.setBaseUrl).toHaveBeenCalledTimes(1)
          expect(context.setBaseUrl).toHaveBeenCalledWith(baseUrl)
        })

        it('should use baseUrl when relative URL is requested', async () => {
          const baseUrl = 'https://www.example.com'
          const relativeUrl = '/cart'
          mstr = ministr({ baseUrl }, context)
          await mstr.visit(relativeUrl)
          expect(fakePage.goto).toHaveBeenCalledTimes(1)
          expect(fakePage.goto).toHaveBeenCalledWith(`${baseUrl}${relativeUrl}`)
        })
      })
    })
  })
